...
func testAccountToManyAccountProfiles(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, accountProfileDBTypes, false, accountProfileColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, accountProfileDBTypes, false, accountProfileColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.AccountProfiles().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = a.L.LoadAccountProfiles(ctx, tx, false, (*[]*Account)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.AccountProfiles); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.AccountProfiles = nil
	if err = a.L.LoadAccountProfiles(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.AccountProfiles); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testAccountToManyAddOpAccountProfiles(t *testing.T) {
	var err error
	...
	foreigners := []*AccountProfile{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, accountProfileDBTypes, false, strmangle.SetComplement(accountProfilePrimaryKeyColumns, accountProfileColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*AccountProfile{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddAccountProfiles(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.AccountProfiles[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.AccountProfiles[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.AccountProfiles().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testAccountToManySetOpAccountProfiles(t *testing.T) {
	var err error
	...
	err = a.SetAccountProfiles(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetAccountProfiles(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.AccountProfiles[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.AccountProfiles[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testAccountToManyRemoveOpAccountProfiles(t *testing.T) {
	var err error
	...
	err = a.AddAccountProfiles(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveAccountProfiles(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.AccountProfiles) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.AccountProfiles[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.AccountProfiles[0] != &e {
...
var (
	accountDBTypes = map[string]string{`ID`: `integer`, `Name`: `character varying`, `Email`: `character varying`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `DeletedAt`: `timestamp without time zone`}
	_              = bytes.MinRead
)
