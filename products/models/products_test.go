...
var (
	productDBTypes = map[string]string{`ID`: `integer`, `Name`: `character varying`, `Price`: `integer`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `DeletedAt`: `timestamp without time zone`}
	_              = bytes.MinRead
)
